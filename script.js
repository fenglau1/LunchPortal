...
const AppState = { user: JSON.parse(localStorage.getItem('lunchUser')) || null, cutoffTime: null, activeVendor: null, selectedDate: null, currentBannerIndex: 0, currentSelection: null, editingOrderId: null, isOrderingForSelf: true, sortOrder: 'asc', sortKey: 'subVendor', bannerInterval: null };
...
const Render = {
    init: async () => {
        ...
        const today = dp ? dp.value || Utils.getTodayDate() : Utils.getTodayDate();
        if (dp) { dp.value = today; dp.addEventListener('change', () => Render.loadDate(dp.value)); }
    ...
    menu: () => {
        ...
        if (subVendors.length > 0) {
            subVendors.forEach(sub => {
                const groupItems = items.filter(i => i.subVendor === sub);
                if (groupItems.length > 0) { html += `<div class="menu-category-wrapper"><div class="category-header"><h4 class="sub-vendor-title">${sub}</h4></div><div class="category-container"><button class="scroll-btn left" onclick="this.nextElementSibling.scrollBy({left:-300, behavior:'smooth'})">❮</button><div class="category-scroll">${groupItems.map(renderCard).join('')}</div><button class="scroll-btn right" onclick="this.previousElementSibling.scrollBy({left:300, behavior:'smooth'})">❯</button></div></div>`; }
            });
            const others = items.filter(i => !subVendors.includes(i.subVendor));
            if (others.length > 0) { html += `<h4 class="sub-vendor-title">Others</h4><div class="menu-grid">${others.map(renderCard).join('')}</div>`; }
        } else { html += `<div class="menu-grid">${items.map(renderCard).join('')}</div>`; }
        container.innerHTML = html;
        // Hide scroll buttons if content fits within container
    ...
    publicOrders: () => {
        ...
        Object.keys(groupedByCategory).forEach(category => {
            ...
            orders.forEach((o, idx) => {
                ...
                const displayUser = o.user !== o.payer ? `${o.user} <small style='color:#aaa'>(${o.payer})</small>` : o.user;
...
const ItemModal = {
    ...
    submitOrder: async () => {
        ...
        let extraCost = 0;
        const selectedAddonNames = []; // We will push Variant Name here too so it shows up in "Order Details" column
